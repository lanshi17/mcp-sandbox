...
def get_container_file(
    sandbox_id: str = Query(..., description="Docker sandbox ID"),
    file_path: str = Query(..., description="Absolute path to the file inside the sandbox, e.g. /app/results/foo.txt")
):
    """
    Read-only access to files inside a running Docker sandbox.
    Returns the file content as a download if found.
    """
    try:
        container = docker_manager.docker_client.containers.get(sandbox_id)
